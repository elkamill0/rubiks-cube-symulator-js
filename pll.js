class PLL {
    ...
    async solve() {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            if (this.cases[key]) {
                ...
                const customAlg = getSelectedAlg('pll', name); // lub 'pll'
    ...
    isSolved() {
        for (let i = 0; i < 4; i++) {
            if (this.cube.edges[i][0]   !== i || this.cube.edges[i][1]   !== 0) return false;
            if (this.cube.corners[i][0] !== i || this.cube.corners[i][1] !== 0) return false;
        }
        return true;
    }
}
